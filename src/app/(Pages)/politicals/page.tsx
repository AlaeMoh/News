...
"use client"
import { fetchGoogleElectricNews, fetchLastPoliticalNews } from '@/app/service/api';
...
export default function Page() {
    const [politicals, setArticles]= useState<Article[]>([])

    useEffect(()=>{
        const fetchData = async ()=>{
            try{
                const lastupdates= await fetchLastPoliticalNews ();
                const google= await fetchGoogleElectricNews()
                setArticles(lastupdates)
  ...
  return (
    <div className='container'>
      <div className="shadow-lg p-3 mb-5 bg-body-tertiary rounded">
       <h1 className='text-danger text-center pt-3'>Political News</h1>
      </div>
